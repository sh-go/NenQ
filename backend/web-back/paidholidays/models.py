...
class PaidHolidays(models.Model):
    ...
    update_start = models.DateField(default=timezone.now)
    update_end = models.DateField(null=True, blank=False)
    ...
    def __str__(self):
        return f"{self.user} - 開始日{self.update_start} - 終了日{self.update_end} - 取得日数{self.date} - 取得時間{self.hour} - {self.text}"
