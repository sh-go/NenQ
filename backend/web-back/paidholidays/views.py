...
from .utils import calculate_paidholiday_summary, monthly_paidholiday_hours
...
class TokenObtainView(jwt_views.TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            res.delete_cookie("access_token", domain=domain)
...
class TokenRefresh(jwt_views.TokenRefreshView):
    def post(self, request, *args, **kwargs):
        ...
        res = Response(serializer.validated_data, status=status.HTTP_200_OK)
        res.delete_cookie("access_token", domain=domain)
        res.delete_cookie("refresh_token", domain=domain)
        res.set_cookie(
            "access_token",
            serializer.validated_data["access"],
            max_age=24 * 60 * 60,  # 24時間
...
class LogoutView(generics.views.APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Logged out incorrectly"}, status=status.HTTP_400_BAD_REQUEST
            )
